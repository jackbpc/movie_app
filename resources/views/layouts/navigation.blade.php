...
<nav x-data="{ open: false }" class="glass-nav sticky top-0 z-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center h-20">
        ...
        <div class="flex items-center space-x-10">
            ...
            <div class="hidden md:flex items-center space-x-6">
                <x-nav-link :href="route('movies.index')" :active="request()->routeIs('movies.index')" class="nav-link-modern">
                    {{ __('All Movies') }}
                </x-nav-link>

                @if(Auth::check() && Auth::user()->role === 'admin')
                    <x-nav-link :href="route('movies.create')" :active="request()->routeIs('movies.create')" class="nav-link-modern">
                        {{ __('Create Movie') }}
                    </x-nav-link>
                @endif
            </div>
        ...
        {{-- Right Section: Search/Sort + User Info --}}
        <div class="flex items-center space-x-6">
            ...
            @if(Auth::check())
                <div class="hidden md:flex items-center space-x-4">
                    ...
                    <a href="{{ route('profile.edit') }}" class="text-sm text-gray-300 hover:text-purple-400 transition">Profile</a>
    ...
    @if(request()->routeIs('movies.index') && isset($navGenres))
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="genre-list">
                @php $currentGenre = request('genre'); @endphp
                <a href="{{ route('movies.index', ['sort'=>request('sort'), 'search'=>request('search')]) }}" 
                   class="{{ is_null($currentGenre) ? 'active' : '' }}">
                    All Genres
                </a>
                @foreach($navGenres as $genre)
                    <a href="{{ route('movies.index', ['genre'=>$genre, 'sort'=>request('sort'), 'search'=>request('search')]) }}" 
                       class="{{ $currentGenre == $genre ? 'active' : '' }}">
                        {{ $genre }}
                    </a>
                @endforeach
            </div>
